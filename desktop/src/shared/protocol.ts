...
// `desktop/src/server/methods.ts` matches exactly one entry in
 ...
 * `desktop/src/server/brain/agent.ts > dispatchNotification`) on the
...
// `desktop/src/server/transport/ws.ts` for the receiver state machine
