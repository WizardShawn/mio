...
async function bootstrap(): Promise<void> {
  ...
  // ─── LAN transports (opt-in) ─────────────────────────────────────
  // Off unless MIO_LAN=1. The transport exposes the same server surface
  // the local renderer drives over IPC, so anything that can pair can
  // drive Mio — read the screen, run tools, spend API credit. Pairing is
  // deliberately permissive for development (any caller that POSTs a
  // deviceId to /pair gets a token), which is fine on a trusted network
  // and emphatically not fine on a shared one. Defaulting to off keeps a
  // stock install from ever binding a port or announcing over mDNS;
  // `getLanPort()` returns null and the shutdown path no-ops.
  if (process.env.MIO_LAN === '1') {
    try {
